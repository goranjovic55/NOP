...
Executes individual workflow blocks with support for:
- Control flow (start, end, delay, condition, loop, parallel, variables)
- Connection testing (SSH, RDP, VNC, FTP, TCP)
- Commands (SSH execute, system info, FTP operations)
- Traffic (ping, capture, storm)
- Scanning (port scan, version detect, network discovery)
- Agent operations (generate, deploy, terminate)
- Data processing (code, output interpreter, assertion, transform)
- Asset operations (list, filter, discover, check online)
"""
...
def _tcp_connection_test(
    host: str, port: int, timeout: int = 5, protocol: Optional[str] = None
) -> BlockExecuteResponse:
    """
    Shared helper for TCP connection tests (SSH, RDP, VNC, FTP, generic TCP).

    Args:
        host: Target hostname or IP
        port: Target port number
        timeout: Connection timeout in seconds
        protocol: Optional protocol name for output metadata
    """
    if not host:
...
async def execute_block(block_type: str, params: Dict[str, Any], context: Dict[str, Any] = None) -> BlockExecuteResponse:
    ...
    try:
        ...
        # === Connection Blocks ===
            ...
            return _tcp_connection_test(
                params.get("host", ""), params.get("port", 5900), params.get("timeout", 5), protocol="vnc"
            )
        
        elif block_type == "connection.ftp_test":
            return _tcp_connection_test(
                params.get("host", ""), params.get("port", 21), params.get("timeout", 5),
                protocol=params.get("protocol", "ftp")
            )
        
        elif block_type == "connection.tcp_test":
            result = _tcp_connection_test(
                params.get("host", ""), params.get("port", 80), params.get("timeout", 5)
            )
            # tcp_test uses "open" key instead of "connected" and always succeeds
            if result.output:
                result.output["open"] = result.output.pop("connected", False)
