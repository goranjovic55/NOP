...
interface KnowledgeEntity {
    type: 'entity' | 'relation' | 'codegraph';
    name?: string;
    entityType?: string;
    observations?: string[];
    from?: string;
...
export class KnowledgeGraphProvider implements vscode.WebviewViewProvider, RefreshableProvider {
    ...
    private sendGraphData() {
        ...
        const knowledgePath = config.get<string>('knowledgeFilePath', 'project_knowledge.json');
    ...
    private parseKnowledgeGraph(content: string): { nodes: any[], links: any[] } {
        ...
        for (const line of lines) {
            try {
                const entry: KnowledgeEntity = JSON.parse(line);
    ...
    private getGroupFromName(name: string): string {
    ...
    private handleEntityClick(entityName: string) {
        if (!this.view) {
            return;
        }
