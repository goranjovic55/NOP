# Feature Design Mockup: Credential Vault & Recent Access
...
This document proposes two new features to enhance user experience:

1. **Credential Vault** - Secure password manager in Access Hub
2. **Recent Access Widget** - Quick access to frequently used assets on Dashboard

---

## 🔐 Feature 1: Credential Vault (Access Hub)
...
├─────────────────────────────────────────────────────────────┤  ┌───────────────────┐  │
│                                                              │  │   VAULT LOCKED    │  │
│  [Tab: SSH 192.168.1.10] [Tab: VNC test-server] [...]      │  │                   │  │
├─────────────────────────────────────────────────────────────┤  │   🔒              │  │
│                                                              │  │                   │  │
│  ┌─────────────────────────────────────────────────────┐   │  │ Enter Password:   │  │
│  │                                                       │   │  │ [_______________] │  │
│  │         Active Connection Display Area               │   │  │                   │  │
│  │                                                       │   │  │  [Unlock Vault]   │  │
│  │                                                       │   │  │                   │  │
│  └─────────────────────────────────────────────────────┘   │  └───────────────────┘  │
│                                                              │                         │
...
│  ┌─────────────────────────────────────────────────────┐   │ ║ 192.168.1.10      ║ │
│  │                                                       │   │ ║ SSH • admin       ║ │
│  │         Active Connection Display Area               │   │ ║ [Connect Now →]   ║ │
│  │                                                       │   │ ╚═══════════════════╝ │
│  │                                                       │   │                       │
│  └─────────────────────────────────────────────────────┘   │ ╔═══════════════════╗ │
│                                                              │ ║ test-vnc          ║ │
...
║ SSH • admin                     ║  ← Protocol • Username (text-cyber-gray-light)
║ [Connect Now →]                 ║  ← Action button (hover: bg-cyber-green)
╚═════════════════════════════════╝
```

- **Hover Effect:** Border color changes to protocol color (SSH=green, VNC=purple, etc.)
...
## 🕒 Feature 2: Recent Access Widget (Dashboard)
...
│ SSH Server           │ ← Asset Description (text-sm, text-cyber-gray-light)
│ • admin              │ ← Username (text-xs, text-cyber-green)
│                      │
│ Last: 2m ago         │ ← Last accessed (text-xs, text-cyber-purple)
│ Uses: 47             │ ← Access count (text-xs, text-cyber-gray)
│                      │
...
- **API Endpoints:**
  - `GET /api/v1/credentials/vault` - List all stored credentials
  - `POST /api/v1/credentials/vault/unlock` - Verify password & unlock
  - `POST /api/v1/credentials/vault/connect` - Quick connect with credential
  - `GET /api/v1/access/recent` - Recent access history
...
### Phase 3: Frontend - Recent Access (Day 3)
...
**Access Hub with Vault:**
- Vault button in top-right toolbar
- Right sidebar (300px) slides in when unlocked
- Password prompt overlay when locked
- Scrollable credential list with search
- Each credential = clickable card with "Connect Now"

**Dashboard with Recent Access:**
- New widget section below stats cards
- Horizontal card layout (3 cards)
- Each card shows: IP, description, username, last access time, usage count
- "Quick Connect" button on each card
