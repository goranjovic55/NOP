# Documentation Index

**Last Updated**: 2026-01-10 | **Total Active Documents**: 49

## Quick Links

| Category | Count | Description |
|----------|-------|-------------|
| [Analysis](#analysis) | 18 | Analysis documents and reports |
| [Architecture](#architecture) | 3 | System design and architecture decisions |
| [Design](#design) | 3 | UI/UX specifications and component library |
| [Development](#development) | 5 | Contributing, testing, and development guides |
| [Features](#features) | 6 | Feature documentation and user guides |
| [Guides](#guides) | 7 | Setup, deployment, and how-to guides |
| [Screenshots](#screenshots) | 1 | Screenshots documentation |
| [Technical](#technical) | 5 | API references and specifications |
| [Archive](archive/INDEX.md) | 53 | Historical documentation (superseded) |

---

## Analysis

Implementation analysis and status reports.

- [**AKIS v7 Framework Audit**](analysis/AKIS_V7_FRAMEWORK_AUDIT.md) ⭐ *New - 100k before/after + microadjustments*
- [**100k Session Simulation Results**](analysis/SIMULATION_100K_RESULTS.md) ⭐ *Comprehensive simulation metrics*
- [AKIS Compliance Audit 100k](analysis/AKIS_COMPLIANCE_AUDIT_100K.md)
- [Custom Agent 100k Simulation](analysis/CUSTOM_AGENT_100K_SIMULATION_REPORT.md)
- [Agent Architecture Gap Analysis](analysis/AGENT_ARCHITECTURE_GAP_ANALYSIS.md)
- [Agent Ecosystem Analysis Report](analysis/AGENT_ECOSYSTEM_ANALYSIS.md)
- [Agent POV Mode Implementation](analysis/AGENT_POV_IMPLEMENTATION.md)
- [Agent SOCKS Proxy Implementation](analysis/AGENT_SOCKS_PROXY.md)
- [Agent Creation Bug Fix](analysis/BUG_FIX_SUMMARY.md)
- [Ecosystem Measurement Analysis](analysis/ECOSYSTEM_MEASUREMENTS.md)
- [Knowledge System Analysis](analysis/KNOWLEDGE_ANALYSIS.md)
- [POV Mode Status](analysis/POV_MODE_STATUS.md)
- [POV Mode with SOCKS Proxy](analysis/POV_SOCKS_USAGE.md)
- [POV Mode Verification](analysis/POV_VERIFICATION_USER_CONFIRMATION_REQUIRED.md)
- [Production Ready Reference](analysis/PRODUCTION_READY.md)
- [AKIS Analysis Reports](analysis/README.md)
- [SOCKS Proxy Implementation Summary](analysis/SOCKS_IMPLEMENTATION_SUMMARY.md)
- [Audit-Optimize Workflow Analysis](analysis/audit-optimize-analysis.md)

## Architecture

System design and technical architecture.

- [System Architecture](architecture/ARCH_system_v1.md)
- [State Management](architecture/STATE_MANAGEMENT.md)
- [Agent C2 Architecture](architecture/agent-c2-architecture.md)

## Design

UI/UX specifications and component library.

- [React Components](design/COMPONENTS.md)
- [UI/UX Mockups](design/UI_UX_SPEC.md)
- [Unified Style Guide](design/UNIFIED_STYLE_GUIDE.md)

## Development

Contributing, testing, and development workflow.

- [Contributing to NOP](development/CONTRIBUTING.md)
- [Development Roadmap](development/ROADMAP.md)
- [Automation Scripts Reference](development/SCRIPTS.md)
- [Testing Guide](development/TESTING.md)
- [Agent/C2 Framework Improvements Roadmap](development/agent-improvements-roadmap.md)

## Features

Feature documentation and user guides.

- [Agent & C2 System](features/AGENTS_C2.md)
- [Feature Proposals](features/FEATURE_PROPOSALS.md)
- [Granular Traffic Filtering](features/GRANULAR_TRAFFIC_FILTERING.md)
- [Implemented Features](features/IMPLEMENTED_FEATURES.md)
- [Storm Functionality](features/STORM_FEATURE.md)
- [Live Traffic Topology](features/live-traffic-topology.md)

## Guides

Setup, deployment, and how-to guides.

- [Agent POV Testing](guides/AGENT-POV-TESTING.md)
- [Agent Testing Guide](guides/AGENT-TESTING-GUIDE.md)
- [Configuration Reference](guides/CONFIGURATION.md)
- [Deployment Guide](guides/DEPLOYMENT.md) ⭐ *Primary deployment reference*
- [Quick Start Guide](guides/QUICK_START.md)
- [Test Environment Setup](guides/SETUP-COMPLETE.md)
- [SOCKS Relay Testing](guides/SOCKS_TESTING_GUIDE.md)

## Screenshots

Visual documentation.

- [Exploit Page Screenshots](screenshots/README.md)

## Technical

API references and specifications.

- [API Specification](technical/API_rest_v1.md)
- [Broadcast Filter Testing](technical/BROADCAST_FILTER_TESTING.md)
- [Backend Services](technical/SERVICES.md)
- [SOCKS E2E Test Results](technical/SOCKS_E2E_TEST_RESULTS.md)
- [SOCKS Integration Complete](technical/SOCKS_INTEGRATION_COMPLETE.md)

---

## Finding Documentation

| Looking for... | Go to |
|----------------|-------|
| Getting started | [Quick Start](guides/QUICK_START.md) |
| Deploying NOP | [Deployment Guide](guides/DEPLOYMENT.md) |
| API endpoints | [API Specification](technical/API_rest_v1.md) |
| System design | [Architecture](architecture/ARCH_system_v1.md) |
| Scripts usage | [Scripts Reference](development/SCRIPTS.md) |
| Historical docs | [Archive](archive/INDEX.md) |

