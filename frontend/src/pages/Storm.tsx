...
const Storm: React.FC = () => {
  ...
  const [portscanPorts, setPortscanPorts] = useState('22,80,443,8080,8443');
  const [portscanResult, setPortscanResult] = useState<any[]>([]);
  ...
  const runPing = async () => {
    ...
    setPingLoading(true);
    try {
      const res = await fetch('/api/v1/storm/ping', {
        method: 'POST',
  ...
  const runTraceroute = async () => {
    ...
    try {
      const res = await fetch('/api/v1/storm/traceroute', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ target: tracerouteTarget })
      });
      const data = await res.json();
  ...
  const runPortscan = async () => {
    ...
    const ports = portscanPorts.split(',').map(p => parseInt(p.trim())).filter(p => !isNaN(p));
    try {
      const res = await fetch('/api/v1/storm/portscan', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ target: portscanTarget, ports })
      });
      const data = await res.json();
      setPortscanResult(data);
    } catch (err) {
      console.error('Portscan failed:', err);
    } finally {
  ...
  return (
    <div className="flex flex-col h-full p-4 space-y-4 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {/* Enhanced Storm Tools Panel */}
        <div className="cyber-panel flex flex-col col-span-1 lg:col-span-2">
          <CyberSectionHeader title="Network Tools" />
          <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Ping Tool */}
            <div className="bg-cyber-dark/50 border border-cyber-gray p-3">
              <div className="text-xs text-cyber-blue font-bold uppercase mb-2">Ping</div>
              <input
                type="text"
                value={pingTarget}
                onChange={(e) => setPingTarget(e.target.value)}
                placeholder="Target IP/hostname"
                className="w-full bg-cyber-darker border border-cyber-gray px-2 py-1 text-cyber-green text-xs font-mono mb-2 focus:border-cyber-blue focus:outline-none"
              />
              <div className="flex gap-2 mb-2">
                <input
                  ...
                  min={1}
                  max={100}
                  className="w-16 bg-cyber-darker border border-cyber-gray px-2 py-1 text-cyber-green text-xs font-mono focus:border-cyber-blue focus:outline-none"
                />
                <span className="text-cyber-gray text-xs self-center">count</span>
              </div>
              <button
                onClick={runPing}
                disabled={pingLoading || !pingTarget}
                className="w-full border border-cyber-blue text-cyber-blue hover:bg-cyber-blue hover:text-black font-bold py-1 px-2 text-xs uppercase transition-all disabled:opacity-50"
              >
                {pingLoading ? 'Pinging...' : 'Run Ping'}
              </button>
              {pingResult && (
                <div className="mt-2 text-xs font-mono text-cyber-gray-light">
                  <div className="flex justify-between">
                    <span>Loss:</span>
                    <span className={pingResult.packet_loss === 0 ? 'text-cyber-green' : 'text-cyber-red'}>
                      {pingResult.packet_loss?.toFixed(1)}%
                    </span>
                  </div>
                  {pingResult.rtt_avg && (
                    <div className="flex justify-between">
                      <span>RTT:</span>
                      <span className="text-cyber-blue">{pingResult.rtt_min}/{pingResult.rtt_avg}/{pingResult.rtt_max} ms</span>
                    </div>
                  )}
                </div>
              )}
            ...
            {/* Traceroute Tool */}
            <div className="bg-cyber-dark/50 border border-cyber-gray p-3">
              <div className="text-xs text-cyber-purple font-bold uppercase mb-2">Traceroute</div>
              <input
                type="text"
                value={tracerouteTarget}
                onChange={(e) => setTracerouteTarget(e.target.value)}
                placeholder="Target IP/hostname"
                className="w-full bg-cyber-darker border border-cyber-gray px-2 py-1 text-cyber-purple text-xs font-mono mb-2 focus:border-cyber-purple focus:outline-none"
              />
              <button
                onClick={runTraceroute}
                disabled={tracerouteLoading || !tracerouteTarget}
                className="w-full border border-cyber-purple text-cyber-purple hover:bg-cyber-purple hover:text-black font-bold py-1 px-2 text-xs uppercase transition-all disabled:opacity-50"
              >
                {tracerouteLoading ? 'Tracing...' : 'Run Traceroute'}
              </button>
              {tracerouteResult && tracerouteResult.hops && (
                <div className="mt-2 max-h-32 overflow-y-auto text-xs font-mono">
                  {tracerouteResult.hops.slice(0, 5).map((hop: any) => (
                    <div key={hop.hop} className="text-cyber-gray-light flex justify-between">
                      <span className="text-cyber-gray">{hop.hop}</span>
                      <span className="text-cyber-purple">{hop.address}</span>
                      <span className="text-cyber-gray">{hop.rtt1}</span>
                    </div>
                  ))}
                  {tracerouteResult.hops.length > 5 && (
                    <div className="text-cyber-gray text-center">+{tracerouteResult.hops.length - 5} more</div>
                  )}
                </div>
              )}
            </div>
            ...
            {/* Portscan Tool */}
            <div className="bg-cyber-dark/50 border border-cyber-gray p-3">
              <div className="text-xs text-cyber-green font-bold uppercase mb-2">Portscan</div>
              <input
                type="text"
                value={portscanTarget}
                onChange={(e) => setPortscanTarget(e.target.value)}
                placeholder="Target IP/hostname"
                className="w-full bg-cyber-darker border border-cyber-gray px-2 py-1 text-cyber-green text-xs font-mono mb-2 focus:border-cyber-green focus:outline-none"
              />
              <input
                type="text"
                value={portscanPorts}
                ...
                className="w-full bg-cyber-darker border border-cyber-gray px-2 py-1 text-cyber-gray-light text-xs font-mono mb-2 focus:border-cyber-green focus:outline-none"
              />
              <button
                onClick={runPortscan}
                disabled={portscanLoading || !portscanTarget}
                className="w-full border border-cyber-green text-cyber-green hover:bg-cyber-green hover:text-black font-bold py-1 px-2 text-xs uppercase transition-all disabled:opacity-50"
              >
                {portscanLoading ? 'Scanning...' : 'Run Portscan'}
              </button>
              {portscanResult.length > 0 && (
                <div className="mt-2 max-h-32 overflow-y-auto text-xs font-mono">
                  {portscanResult.map((r: any) => (
                    <div key={r.port} className="flex justify-between">
                      <span className="text-cyber-gray">{r.port}</span>
                      <span className={r.status === 'open' ? 'text-cyber-green' : 'text-cyber-gray'}>
                        {r.status}
                      </span>
                    </div>
                  ))}
